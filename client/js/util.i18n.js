...
const LANGUAGES = {
	en: {
		...
		translations: {
			// Meta tags for SEO
			'meta.description': 'NodeCrypt - End-to-end encrypted chat with client-encrypted SQLite group history, Cloudflare Workers, Docker, self-hosting and local development.',
			...
			'ui.username_taken': 'This username is already in use in the room',
			'ui.join_rejected': 'Unable to enter the room',
			'ui.node_unreachable': 'Unable to connect to the selected node. Check its IP, firewall, and port.',
			'ui.my_name': 'My Name',
			'ui.members': 'Members',
			'ui.message': 'Message',
			'auth.login': 'Login',
			...
			'history.local_password_mismatch': 'Local history exists for this room, but the room password does not match',
			'history.node_password_mismatch': 'History exists on this node, but the room password does not match',
			'history.local_load_failed': 'Failed to read local history',
			'history.node_load_failed': 'Failed to read history from this node',
			'history.decrypt_failed': 'Some history could not be decrypted; verify that the room password has not changed',
			'ui.private_message_to': 'Private Message to',
			'ui.me': ' (me)',
			'ui.anonymous': 'Anonymous',
			'ui.start_private_chat': 'Select for private chat',
			'ui.current_user': 'You',
			'ui.member_connecting': 'Connecting...',
			'ui.private_chat': 'Private',
			'ui.offline': 'Offline',
			'ui.close_private': 'Close private chat',
			...
			'help.no_history': '🔒 Encrypted Group History',
			'help.no_history_desc': 'Group text, images, and file volumes are stored as client-encrypted SQLite records; private conversations remain session-only',
	...
	zh: {
		...
		translations: {
			// Meta tags for SEO
			'meta.description': 'NodeCrypt - 支持客户端加密 SQLite 群聊记录的端到端加密聊天系统，支持 Cloudflare Workers、Docker、自托管和本地开发。',
			...
			'meta.og_description': 'NodeCrypt 是一个端到端加密的开源聊天系统，群聊记录在客户端加密后保存到 SQLite，服务器无法获取明文。',
			...
			'ui.username_taken': '该用户名已在房间中使用，请更换用户名',
			'ui.join_rejected': '无法进入该房间',
			'ui.node_unreachable': '无法连接所选节点，请检查对方 IP、防火墙和端口',
			'ui.my_name': '我的名字',
			'ui.members': '在线成员',
			'ui.message': '消息',
			'auth.login': '登录',
			...
			'auth.username_exists': '账号名称已存在',
			'auth.too_many_attempts': '尝试次数过多，请稍后再试',
			'auth.request_failed': '请求失败',
			'history.local_password_mismatch': '本机存在该房间的历史记录，但房间密码不一致',
			'history.node_password_mismatch': '当前节点存在该房间的历史记录，但房间密码不一致',
			'history.local_load_failed': '本机历史记录读取失败',
			'history.node_load_failed': '当前节点历史记录读取失败',
			'history.decrypt_failed': '部分历史记录无法解密，请确认房间密码未变更',
			'ui.private_message_to': '私信给',
			'ui.me': '（我）',
			'ui.anonymous': '匿名用户',
			'ui.start_private_chat': '选择用户开始私信',
			'ui.current_user': '当前用户',
			'ui.member_connecting': '正在建立连接...',
			'ui.private_chat': '私聊',
			'ui.offline': '离线',
			'ui.close_private': '关闭私聊',
			...
			'help.what_is_nodecrypt': '🔐 什么是 NodeCrypt？',			'help.what_is_nodecrypt_desc': 'NodeCrypt 是一个端到端加密聊天系统。群聊记录在您的设备上加密后保存到 SQLite，服务器只接触密文，无法获取明文内容。',
			...
			'help.no_history_desc': '群聊文本、图片和文件分卷以客户端密文保存到 SQLite；私聊仍只存在于当前会话',
